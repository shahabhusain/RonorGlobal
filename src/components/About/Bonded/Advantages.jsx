...
const Advantages = () => {
  ...
  return (
    <div className='w-[95%] mx-auto mt-20'>
      ...
      <h2 className='text-[#00105C] xl:text-[40px] lg:text-[35px] md:text-[30px] text-[25px] font-[800]'>Highlights</h2>
        ...
        {servicesData.map((item, index) => (
          ...
          >
            <div className='flex flex-col group-hover:flex-row items-center gap-3 group-hover:mt-12 transition-all duration-300'>
              <h1 className='md:text-[25px] text-[16px] flex items-center justify-center font-[700] text-center transition-colors duration-300 text-[#00105C] group-hover:text-white'>
                {item.title}
              </h1>
            </div>
            <p className='md:text-[18px] text-[15px] group-hover:text-center font-[500] hidden group-hover:block text-center transition-colors duration-300 text-black group-hover:text-white'>
              {item.desc}
            </p>
          </div>
        ))}
      ...
      <div className='md:hidden mt-12 overflow-x-auto'>
        <div className='flex space-x-4 pb-4' style={{ width: `${servicesData.length * 280}px` }}>
          {servicesData.map((item, index) => (
            <div
              key={index}
              className='flex flex-col gap-4 items-center border-[2px] w-[270px] h-[300px] bg-[#BDC6F2] border-[#7A7A7A] py-8 px-3 rounded-md'
            >
              <div className='flex flex-col items-center gap-3'>
                <h1 className='text-[25px] flex items-center justify-center font-[700] text-center text-[#00105C]'>
                  {item.title}
                </h1>
              </div>
              <p className='text-[18px] text-center font-[500] text-black'>
                {item.desc}
              </p>
            </div>
          ))}
