      ...
      <div className='border-t-4 lg:border-t-0 lg:border-l-4 border-[#7A7A7A] w-full lg:w-[40%] p-4'>
        ...
        <div className='hidden lg:block w-full h-[250px] xl:h-[200px] overflow-y-auto scrollbar-custom pr-2'>
          {newsItems.map((item, index) => (
            <div key={index} className='border-b-[2px] border-black pb-4 mt-4'>
              ...
              <button className='flex items-center gap-2 mt-2 text-sm'>
                {item.date} <Link to={`blogdetail/${item.id}`} className='text-red-500 hover:text-red-600 underline transition-colors duration-200'>Know more</Link>
              </button>
            </div>
          ))}
        </div>
            ...
            >
              {newsItems.map((item) => (
                ...
                >
                  ...
                  <button className='flex items-center gap-2 mt-2 text-sm'>
                    {item.date} <spa className='text-red-500 hover:text-red-600 underline transition-colors duration-200'>Know more</spa>
                  </button>
                </div>
              ))}
            </div>
