...
const Services = () => {
  ...
  return (
    <div className=' w-[95%] mx-auto pt-20'>
        ...
        <div className=' grid grid-cols-5 gap-x-5 mt-12'>
         {
            servicesData.map((item, index)=>(
                <div key={index} className=' flex flex-col gap-4 items-center border-[2px] border-[#7A7A7A] py-8 px-3 rounded-md relative'>
                <img className=' w-[70px]' src={item.img} alt="" />
                <h1 className=' text-[25px] font-[700] text-[#00105C] text-center'>{item.title}</h1>
                <p className=' text-[18px] font-[500] text-black text-center'>{item.desc}</p>
